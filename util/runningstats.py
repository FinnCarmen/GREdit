...
class SecondMoment(Stat):
    ...
    def add(self, a):
        ...
        self.mom2 += a.t().mm(a)
        # if self.count == 0:
        #     self.mom2 = torch.zeros(a.shape[1], a.shape[1], device=a.device, dtype=torch.float64)

        # a64 = a.to(torch.float64)
        # self.mom2 += a64.t().mm(a64)
        # self.count += a.shape[0]
        # 检查是否存在inf
        if torch.isinf(self.mom2).any():
            raise ValueError("SecondMoment encountered Inf values in mom2.")
    ...
    def moment(self):
        return self.mom2 / float(self.count)
